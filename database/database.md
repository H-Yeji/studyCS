### ☺️아래 질문에 대한 대답은 '면접'을 위한 답변입니다 ! 상세 내용은 노션을 확인해주세요☺️
...
이런 이상 연상을 제거하면서 데이터베이스를 올바르게 설계해나가는 과정을 ‘정규화’라고 합니다. 
```
...
이 과정을 거친 후 모든 컬럼은 기본키에만 의존하도록 변경됩니다.
...
이 과정을 거치면 모든 관계가 더 이상 분해될 수 없도록 테이블을 분해해 중복을 제거합니다.
```
